...
class BackstabberAPIHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        try:
            ...
            if path == "/api/overview":
                self._json(self._control_plane().overview())
                return
            if path == "/api/engagements":
                self._json(self._control_plane().list_engagements())
                return
            if path.startswith("/api/engagements/") and path.endswith("/assets"):
                engagement_id = self._path_part(path, 2)
                self._json(self._control_plane().list_assets(engagement_id))
                return
            if path.startswith("/api/engagements/"):
                engagement_id = self._path_part(path, 2)
                self._json(self._control_plane().get_engagement(engagement_id))
                return
            if path == "/api/jobs":
                status = query.get("status", [None])[0]
                self._json(self._control_plane().list_jobs(status=status))
                return
            if path == "/api/approvals":
                status = query.get("status", [None])[0]
                self._json(self._control_plane().list_approvals(status=status))
                return
            if path == "/api/executions":
                engagement_id = query.get("engagement_id", [None])[0]
                self._json(self._control_plane().list_executions(engagement_id=engagement_id))
                return
            if path == "/api/audit":
                limit = int(query.get("limit", ["100"])[0])
                self._json(self._control_plane().list_audit(limit=limit))
                return
    ...
    def do_POST(self) -> None:
        try:
            ...
            if path == "/api/engagements":
                self._json(
                    self._control_plane().create_engagement(
            ...
            if path == "/api/jobs/dry-run":
                plan = self._control_plane().build_plan(
                    engagement_id=str(body.get("engagement_id") or ""),
                    operation=str(body.get("operation") or ""),
                    payload=self._payload(body),
                    dry_run=True,
                )
                execution = self._control_plane().record_dry_run(
                    engagement_id=str(body.get("engagement_id") or ""),
                    operation=str(body.get("operation") or ""),
            ...
            if path == "/api/jobs":
                self._json(
                    self._control_plane().enqueue_job(
                        engagement_id=str(body.get("engagement_id") or ""),
                        operation=str(body.get("operation") or ""),
                        payload=self._payload(body),
                        actor=actor,
            ...
            if path.startswith("/api/approvals/") and path.endswith("/approve"):
                ...
                self._json(self._control_plane().approve_job(approval_id, actor=actor))
                return

            if path.startswith("/api/approvals/") and path.endswith("/reject"):
                approval_id = self._path_part(path, 2)
                self._json(
                    self._control_plane().reject_job(
                        approval_id,
                        actor=actor,
    ...
    def finish(self) -> None:
        try:
            ...
            plane = getattr(self, "_request_control_plane", None)
    ...
    def _control_plane(self) -> ControlPlane:
        plane = getattr(self, "_request_control_plane", None)
        if plane is None:
            plane = ControlPlane(db=Database(self.db_path))
            self._request_control_plane = plane
        return plane
